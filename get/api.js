'use strict'
// APIs = Es una forma de comunicarse entre aplicaciones. Una aplicacion realiza una peticion a otra, y esta le tiene que responder. Hay una peticion del cliente, una espera y el servidor nos devuelve los datos. EJ = En la aplciacion del clima, tiene una API de una pagina meteorologica.
// En las API, siempre usamnos GET que es para hacer consultas a otros servidores. Los datos de las apis se reciben en objetos con su informacion.
// const usuarios ={
//     "users": [
//     {
//     "id": 1,
...
//     // }
//     console.log(`Nombre : ${usuario.firstName}`);
//     console.log(`Pelo : ${usuario.hair.color}`);
//     console.log(">>>>>>>>>>>>>>>>>");
// }
...
setTimeout(() => {
    if (edad > 30) {
        console.log(`${nombre} es viejo pero sigue siendo el mejor del mundo, mejor que cristiano el baby ese`);
        mayorDeEdad = true;
    }else{
        console.log(`${nombre} Es la pulgaaaa`);
        mayorDeEdad = false;
    }
}, 10000);

console.log(`${nombre} es ${mayorDeEdad}`);
// El codigo sincronico se ejecuta linea por linea, en orden y se tiene que ejecutar una tarea antes que otra para que funcione, va una en una
// Siempre antes de ejecutar el codigo, debemos esperar que los datos de la API lleguen, ya que si tarda en llegar, el codigo se ejecuta sin datos.

function verifiacrEdad(nombre,edad,callback){
    setTimeout(() => {
        if (edad > 30) {
            console.log(`${nombre} es viejo pero sigue siendo el mejor del mundo, mejor que cristiano el baby ese`);
            mayorDeEdad = true;
...
verifiacrEdad(nombre,edad,fin);

function pizza() {
    return new Promise((resolve,reject) => {
        const exito = Math.random() * 3 < 2
        setTimeout(() => {
            if (exito) {
                resolve("Pizza hecha");
            }else{
...
pizza().then(mensaje => console.log("Operacion confirmada :",mensaje)).catch(eror => console.log("Operacion mala",eror))
